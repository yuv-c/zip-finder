...
resource "aws_instance" "es_kibana_instance" {
  ...
  user_data = <<-EOF
              ...
              sudo docker run -d --name elasticsearch -p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" -v es_data:/usr/share/elasticsearch/data docker.elastic.co/elasticsearch/elasticsearch:7.10.2
              sudo docker run -d --name kibana --link elasticsearch:elasticsearch -p 5601:5601 -v kibana_data:/usr/share/kibana/data docker.elastic.co/kibana/kibana:7.10.2
              ...
              # Verify installation
              sudo docker exec elasticsearch ./bin/elasticsearch-plugin list
